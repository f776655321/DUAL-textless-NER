        ...
        if(isinstance(ner_tags, float) == True):
            ...
            for ner_tag in ner_tags:
                ...
                if(ner_tag[0] not in NER_Label_dict):
                    ...
                    new_label = NER_Label_dict[ner_tag[0]]
                    if new_label not in exist_label:
                        exist_label.add(new_label)
                        find_time[new_label] = set()
                    ...
                    for word in word_segments:
                        ...
                        if( first_target == True and (target[end_index] == word.label or target[end_index] == word.label.replace("'s",''))):
                            ratio = waveform.size(1) / (trellis.size(0) - 1)
                            x1 = int(ratio * word.end)
                            word_end_index = word.end
                            second_target = True
                        
                        if(second_target == True and (word_start_index,word_end_index) not in find_time[new_label]):

                            start_time = x0 / bundle.sample_rate
                            end_time = x1 / bundle.sample_rate
                            start_store[label2index[new_label]].append(round(start_time,3))
                            end_store[label2index[new_label]].append(round(end_time,3))
                            find_time[new_label].add((word_start_index,word_end_index))
                            not_find = False
                            break

                        elif(second_target == True and (word_start_index,word_end_index) in find_time[new_label]):
                            first_target = False
                            second_target = False
                    if(not_find == True):
                        unsccessful_alignment += 1
                        start_store[label2index[new_label]].append(0)
                        end_store[label2index[new_label]].append(0)

            not_exist = [tag for tag in Combined_label if tag not in exist_label]
            for tag in not_exist:
                start_store[label2index[tag]].append(0)
                end_store[label2index[tag]].append(0)
            
            start.append(start_store)
            end.append(end_store)

    df['context_id'] = ans_context_id
    df['start'] = start
    df['end'] = end
    print('number of not successful alignment: ',unsccessful_alignment)
    df.to_csv('./new-data/' + mode +'_just_for_test.csv', index=False)
