	...
	if True:
		for mode in ["train", "validation"]:
			if mode == "train":
				...
				df = pd.read_csv("code-data-sa/train_code_ans_balance.csv")    
				# df = pd.read_csv("code-data/train_code_ans_sampling_negative_2.csv")    
			elif mode == "validation":
				df = pd.read_csv("code-data-sa/validation_code_ans.csv")   
				
			code_dir = os.path.join("code-data-sa", 'question-code-SA')
			# TODO
			code_passage_dir = os.path.join("code-data-sa", 'code/' + mode + '/')
			...
			action_code = {}
			candidate_code = []
			root_dir = "code-data-sa/question-code-SA"
			...
			for context_id, label, start_idx, end_idx in tqdm(zip(context_ids, labels, code_starts, code_ends), total=len(context_ids)):
				...
				for i in range(100):
					raw_dataset["hash"].append("")
					raw_dataset["text"].append("")
					raw_dataset["utterance"].append("")
					raw_dataset["new_start"].append(0.00)
					raw_dataset["new_end"].append(0.00)
					raw_dataset["question_file"].append("")
					raw_dataset["context_file"].append("")
				...
				idx += 1
			raw_datasets_sa = Dataset.from_dict(raw_dataset)

			if mode == "train":
				raw_datasets_qa = load_dataset(
					"Splend1dchan/NMSQA_testupload2",
					split=mode,
				)
			elif mode == "validation":
				raw_datasets_sa = load_dataset(
					"Splend1dchan/NMSQA_testupload2",
					split="dev"
				)
			raw_datasets[mode] = concatenate_datasets([raw_datasets_sa, raw_datasets_qa])
