...
data_dir = '/tmp2/b09902111/DUAL-textless-NER'


mode = 'dev'

audio_dir = '/tmp2/b09902111/slue-voxpopuli/dev/'

df = pd.DataFrame()
sluedf = pd.read_csv('/tmp2/b09902111/slue-voxpopuli/slue-voxpopuli_dev.tsv',sep='\t')
...
for i , ner_tags in enumerate(tqdm(normalized_ner)):
    ...
    if(isinstance(ner_tags, float) == False):
...
df.to_csv('./new-data/dev_ans.csv', index=False)
