...
parser.add_argument('--output_dir', default='./search_result', type=str)
...
class SQADevDataset(Dataset):
    def __init__(self, data_dir):
        ...
        code_dir = os.path.join(data_dir, 'question-code-chatgpt-short/')
...
for threshold in threshold_search_space:
    for answer_length in answer_length_search_space:
        ...
        for index, FF1 in enumerate(FF1s):
            if FF1 > BestFF1ThersholdForTags[index][5]:
...
output_df.to_csv(os.path.join(args.output_dir,args.output_fname),index=False)
